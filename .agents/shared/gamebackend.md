...
1. **Foundations (`core/*`) never depend on a module or an `api/` crate.**
   ...
   edge, lifecycle, opsapi, registry, asyncevents, invalidation, remote,
...
- **gateway** — the front-door module: HTTP ops routing (Local vs Remote
  ...
  itself never reads env), player-QUIC plane (bearer-in-envelope,
  exact-method allow-list), HTTP passthrough (`/admin`, `/accounts/epic` →
  origins passed in by `cmd/gateway-svc` via `Gateway::with_passthrough`,
