...
/// Migrates BOTH the messaging (durable transport's outbox) and match schemas EXACTLY
...
/// filling match's `MmrReader` dependency: schemas migrated once, messaging's phase-1
/// `register` installs the transport, rating provides `MmrReader`, then match
/// registers/inits against the same ctx. Returns the ctx (owns the bus + registry) and
/// the wired match service.
...
async fn cleanup(pool: &PgPool, match_id: &str) {
    let _ = sqlx::query("DELETE FROM match.matches WHERE id = $1::uuid")
        ...
        .execute(pool)
        .await;
    let _ = sqlx::query("DELETE FROM messaging.outbox WHERE payload->>'match_id' = $1")
        .bind(match_id)
        .execute(pool)
        .await;
}
...
async fn outbox_count(pool: &PgPool, match_id: &str) -> i64 {
    let (n,): (i64,) = sqlx::query_as(
        "SELECT count(*) FROM messaging.outbox WHERE topic = 'match.finished' AND payload->>'match_id' = $1",
    )
    .bind(match_id)
    .fetch_one(pool)
    .await
...
/// THE ATOMIC EMIT PROOF: `report` writes BOTH a `match.matches` row AND a
/// `messaging.outbox` row (topic `match.finished`) in one tx — proving `emit_tx` rode
...
/// `requires(["rating","messaging"])`, and the missing sync dependency fails loud at
/// startup (CLAUDE.md's hard constraint), never a silent nil-service at report time. No
/// DB needed — this is a pure manifest check over the static module list.
#[test]
fn match_requires_rating_and_fails_validate_without_it() {
    // Only messaging + match (no rating provider): validate_requires must reject it.
    let mods: Vec<Box<dyn Module>> = vec![
        Box::new(MatchModule::new()),
