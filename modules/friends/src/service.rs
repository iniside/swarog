...
/// The ONE answer for an edge that does not exist, an edge the caller is not party to, and
/// an edge whose state no longer admits the operation. A 403 would confirm that the id names
/// a real relation the caller is not part of, which is an enumeration oracle.
...
/// one `store`'s `CREATED_TEXT` renders.
...
/// checked HERE and the statement has no timestamp arm to map: a digit-SHAPED but impossible
/// instant (`2026-13-45`, hour 25, year 0000) is `Status::Invalid` like any other malformed
/// cursor, where letting it through would raise 22008 and answer 500 — contradicting the
/// contract's own promise on `Player::list`.
///
/// Deliberately duplicated from `notifications`' codec rather than shared: a module may not
/// import another module's impl crate, and neither cursor's shape is a contract surface.
...
/// The opaque page token: base64url-no-pad over `"{created_at}|{edge_id}"`, the keyset the
...
/// silent reset makes a paging bug look like a working list that repeats its newest page.
///
/// The cap is checked BEFORE the decode, as the contract states.
///
...
/// Every directory failure is `Status::Unavailable` (503), never a page of blank names and
/// never a 500: the two inputs this module passes — a handle already checked against
/// [`MAX_HANDLE_BYTES`] and at most `MAX_PAGE_LIMIT` canonical ids — cannot be the malformed
/// request the capability rejects, so an `Err` from it is the directory being unreachable.
/// A directory MISS is the opposite outcome and never reaches here: it is an absent
/// summary, which keeps its row with empty name and handle.
fn directory_unavailable(e: Error) -> Error {
    Error::unavailable(format!("player directory unavailable: {}", e.msg))
}

/// A `22P02` on the pair-addressed write path is the CALLER's identity, not a client-typed
/// id: the target's id came from the directory. It is a 400, not the 500 the raw SQLSTATE
/// would produce.
...
/// The one place the table's `state` vocabulary meets the contract's, so a value the CHECK
/// constraint would have to admit for it to exist can never reach the wire unlabelled.
...
/// One party of a relation as an event payload needs it: the DB-canonical id plus the handle
/// SNAPSHOT taken at emit time. A player the directory does not return keeps an empty handle
/// — the same miss semantics the read path serves.
struct Party {
    id: String,
    handle: String,
}
...
fn party(summaries: &[PlayerSummary], id: &str) -> Party {
    match summaries.iter().find(|s| s.player_id.eq_ignore_ascii_case(id)) {
        Some(s) => Party {
...
/// The row→DTO mapping is CALLER-RELATIVE and therefore not a `From<EdgeRow>`: `direction`
/// says who asked *relative to the caller*, and the row's other columns describe the OTHER
/// player.
fn friend_of(
    edge_id: String,
    state: &str,
...
pub struct Service {
    ...
    /// The mandatory `accounts` directory, resolved in `init` (phase 2). In a split process
    /// a `remote::Stub` swaps an edge-backed client under the same key.
...
impl Service {
    ...
    /// Both parties' canonical ids and handles, resolved in ONE batched call BEFORE any
    /// transaction opens: a directory RPC issued while holding one would pin the connection
    /// and the row's locks across the network, so one accounts blip would stall writers.
    ...
    async fn page(
        ...
        for row in rows {
            let other = summaries.iter().find(|s| s.player_id == row.other_id);
...
impl Player for Service {
    /// Three branches, decided by two statements rather than by a duplicate-key exception:
    /// a conflict on the pair index has three distinct causes, and turning it blindly into
    /// an acceptance would let a player accept their OWN request with a second POST.
    ///
    /// The insert emits `friend.requested`, the crossing update emits `friend.accepted`, and
    /// the third branch — the caller's own request repeated, or one already accepted —
    /// emits NOTHING and answers with the relation as it stands.
    async fn request(&self, identity: Identity, target_handle: String) -> Result<Friend, Error> {
        let me = Service::caller(&identity)?;
        // `accountsapi`'s cap is the one authority for a handle's size; friends declares no
        // second const and never hands an over-long handle to the directory.
        ...
        // Compared on the DIRECTORY's canonical ids, so a caller cannot address itself by
        // spelling its own id differently.
        if me_summary.id.eq_ignore_ascii_case(&target.player_id) {
        ...
        {
            // Counted AFTER the insert, in its transaction: a caller at the cap repeating an
            // existing request takes one of the two branches below and still answers 201, as
            // the contract promises — only a NEW relation can be refused.
            let outstanding = self
                .store
                .count_authored_pending_tx(&mut tx, &me, STATE_PENDING)
                .await
                .map_err(internal)?;
            ...
            let evt = friendsevents::Requested {
                ...
                requester_id,
                requester_handle: me_summary.handle,
        ...
        match existing {
            ...
            // The pair's row was removed between the conflicting insert and this read, so
            // there is nothing to report as current — retrying creates it.
            None => Err(Error::conflict("friend request raced a concurrent removal")),
        }
    }
    ...
    async fn remove(&self, identity: Identity, edge_id: String) -> Result<(), Error> {
        ...
        // Which ending this is, decided from the DELETED row: ending a friendship and
        // cancelling an offer say opposite things about the graph, and a consumer keeping a
        // pending-versus-friends count decrements a different number for each.
    ...
    /// a client cannot page another player's social graph.
