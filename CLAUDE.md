...
- **audit** — append-only ledger (`audit.log`), zero-coupling raw durable sinks for
  all 11 ledger topics (`character.created/deleted`, `player.registered`,
  `player.promoted`, `config.changed`, `match.finished`, `admin.action`,
  `wallet.changed`, `friend.requested/accepted/removed`) — eleven
  independent subscriptions (`audit.<topic-kebab>.v1`), each
  with its own checkpoint, plus a 12th independent subscription for prune reacting
...
mail :8094/:9012, friends :8095/:9014. The fleet
...
[P1-P6], friends [FR1-FR9]), then re-runs the monolith (`cmd/server`) on the same player front for parity
