...
UILayout/                  # Codex Design mockups (spec for admin UI, not runnable)
```
...
default (which hardcodes Opus 4.8): Opus → `Codex Opus 4.8`, Fable → `Codex Fable
5`, Sonnet subagent → `Codex Sonnet 4.6` (all `<noreply@anthropic.com>`). When
