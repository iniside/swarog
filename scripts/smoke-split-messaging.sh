#!/usr/bin/env bash
# smoke-split-messaging.sh — end-to-end proof that the DURABLE EVENT PLANE (the
# bus-owned messaging transport) works in the MICROSERVICES SPLIT, and that the
...
#
#   1. inventory-svc + scheduler-svc booted (they Require("messaging"); the binary
#      would refuse to boot via validateRequires if messaging were absent).
#   2. DURABLE CROSS-PROCESS DELIVERY: a character created on characters-svc (A)
#      lands a starter grant in inventory on inventory-svc (B) — i.e.
#      character.created flowed A's domain tx -> messaging.outbox(origin=
#      characters-svc) -> A's relay -> POST /events -> B's inbound sink ->
#      inventory.grantStarter, all with NO module knowing the topology.
#   3. BLOCKER-1 REGRESSION (the whole redesign's reason to exist): with
#      scheduler-svc (origin=scheduler-svc) running its OWN relay over the SAME
#      shared messaging.outbox, A's character.created row is NOT swallowed by a
#      foreign-origin relay. Proven two ways: the grant lands (assertion 2), AND
...
# Run from repo root: ./scripts/smoke-split-messaging.sh
set -uo pipefail
cd "$(dirname "$0")/.."
...
# --- assertion 1: messaging is hosted where producers/consumers live -------
grep -qE 'msg="module ready" module=messaging' run/inventory.out.log \
    || fail "inventory-svc did not report messaging hosted — validateRequires would have failed"
pass "inventory-svc booted WITH messaging hosted (durable consumers satisfied)"
grep -qE 'msg="module ready" module=messaging' run/scheduler.out.log \
    || fail "scheduler-svc did not report messaging hosted"
...
# --- assertion 2: the grant lands on B via the durable plane ---------------
GRANTED=""
t=40
while [ "$t" -gt 0 ]; do
...
# --- assertion 3: BLOCKER-1 — the row is A's origin, consumed by inventory, not swallowed
# The outbox row for THIS character's created event: stamped origin=characters-svc.
OID=$(psql_q "SELECT id FROM messaging.outbox WHERE topic='character.created' AND payload->>'CharacterID' = '$CID' LIMIT 1;")
[ -n "$OID" ] || fail "no messaging.outbox row found for character.created of $CID"
OORIGIN=$(psql_q "SELECT origin FROM messaging.outbox WHERE id = $OID;")
...
t=40
while [ "$t" -gt 0 ]; do
    if [ "$(psql_q "SELECT sent_at IS NOT NULL FROM messaging.outbox WHERE id = $OID;")" = "t" ]; then
        OSENT=t; break
    fi
    t=$((t - 1)); sleep 0.25
done
...
t=40
while [ "$t" -gt 0 ]; do
    INBOX=$(psql_q "SELECT string_agg(subscriber, ',' ORDER BY subscriber) FROM messaging.inbox WHERE event_id='$EVID';")
