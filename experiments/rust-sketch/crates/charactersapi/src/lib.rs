...
//! `Method*` consts, a `Client` over `opsapi::Caller`, `register_server`, and — for
//! `#[http]` methods — `operations()`/`route_bindings()`) is GENERATED into child
//! `*_rpc` modules rather than hand-written.
//!
