...
- [Durable event plane is bus-owned](durable-event-plane-bus-owned.md) — outbox/inbox/relay/HTTP hidden behind bus via modules/messaging (Emit/EmitTx/On/OnTx/OnTxRaw); modules never branch on topology; single-owner relay by origin. Don't re-propose the old per-module sink pattern
- [Unified operation transport (sync)](unified-operation-transport.md) — sync capabilities are typed ops (opsapi/rpcgen glue, consumers keep local ifaces); gateway fronts every process via leaf slot + is the client; auth once over mTLS edge; admin fan-out folded; /events bypasses gateway. gateway-svc single-front-door DONE (dispatches ops over edge, double-layer gone); Go's :9100 QUIC front still unauthenticated — the RUST sketch closed that gap 2026-07-08
