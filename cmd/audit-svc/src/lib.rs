//! Library half of `audit-svc`'s composition root (Step 10): the real module list,
...
/// audit-svc dials no peer (it only SERVES `admin.adminData` over its own edge and
/// pulls durable events), so the accepted `wiring` is unused today — the parameter
