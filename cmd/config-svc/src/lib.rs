//! Library half of `config-svc`'s composition root (Step 10): the real module list,
...
/// config-svc dials no peer (it only SERVES `config.snapshot` over its own edge), so
/// the accepted `wiring` is unused today — the parameter exists so every `cmd/*-svc`
/// lib shares the one `modules(&ProcessWiring)` signature `tools/checkmodules`
/// depends on.
