...
//! hosts NO edge server of its own — it only DIALS the twelve peers (characters,
...
//! ports, including `NOTIFICATIONS_EDGE_ADDR`/`MAIL_EDGE_ADDR`/`GROUPS_EDGE_ADDR`); the
//! shared dev CA (`EDGE_CA_CERT`/`EDGE_CA_KEY`) authenticates the dials.
...
async fn main() -> anyhow::Result<()> {
    ...
    let wiring = ProcessWiring::new()
        ...
        .with_peer("groups", env_addr("GROUPS_EDGE_ADDR", "127.0.0.1:9015"));
