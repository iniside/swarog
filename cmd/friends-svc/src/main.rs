//! `friends-svc` — the friends fortress process. It hosts friends and fills its
//! `accounts` dependency (`accountsapi::Directory`) with a `remote::Stub`: it `provide`s
//! an edge-backed client under the SAME registry key the local impl would, so friends'
//! `require::<dyn Directory>` resolves REMOTELY — the registry SWAP, with friends' code
//! unchanged.
//!
...
//! consumes `accounts.directory` purely as a sync capability. It serves `friends.*`
...
//! event append (`friend.requested`/`accepted`/`removed`) rides the shared log this
//! process's own DB pool writes into, atomically with the domain row.
...
async fn main() -> anyhow::Result<()> {
    ...
    // One shared QUIC edge server for this process. `friends` contributes its
    // `friends.*` face to `edge::EDGE_SLOT` during `init`; `app::run` applies the
    ...
    let mods = friends_svc::modules(&wiring);

    // Serves friends ops on its own edge (`EDGE_ADDR`, default `:9014` in the run
    // scripts) so gateway-svc can dispatch `friends.*` Remote to it. No player front
    // here either way — this process is fronted by gateway-svc, never directly by
    // players.
