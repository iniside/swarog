...
/// Reads the `/push` WebSocket bounds from the environment. Env is read HERE in the
/// composition root (the gateway module never reads it); the PARSING — including which
/// names exist and which values fail startup — is `gateway::PushLimits::from_values`,
/// one authority both front mains drive, so the two roots cannot drift.
fn push_limits_from_env() -> anyhow::Result<gateway::PushLimits> {
    gateway::PushLimits::from_values(|name| std::env::var(name).ok())
}
...
async fn main() -> anyhow::Result<()> {
    ...
    // Managed boot (A5 + C2): hand each edge stub a live LIST re-resolver bound to the
    // agent URL, so its capability caller is a `remote::Pool` that round-robins across the
    // provider's live instances and picks up a moved peer OR a scale event without
    // restarting this front door. Standalone boot (`AddrSource::Env`, `agent_url()` is
    // `None`) passes `None` — a single fixed conn, byte-identical to before. The boot
    // snapshot the gateway route table reads was already resolved once above by
    // `gateway_addrs` (the whole instance set per edge peer).
    let edge_mk = if source.agent_url().is_some() {
        Some(move |provider: &'static str| addrs::edge_list_resolver(&agent_url, provider))
    } else {
        None
    };
