...
//! leaderboard OWNS a schema + an inbox, so this process needs a DB pool and thus hosts
//! the durable-events plane (app-owned, DB ⇒ plane): its `POST /events` inbound sink
//! receives `match.finished` from match-svc's relay, and leaderboard's durable `on_tx`
//! upserts wins+1 on the handed inbox-dedup tx.
//!
