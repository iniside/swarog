...
/// notifications-svc dials no peer (its `requires()` is empty — retention is a config
/// env var, not a synchronous capability), so the accepted `wiring` is unused today —
/// the parameter exists so every `cmd/*-svc` lib shares the one
/// `modules(&ProcessWiring)` signature `tools/checkmodules` depends on.
