//! Library half of `rating-svc`'s composition root (Step 10): the real module list,
...
/// rating-svc dials no peer (it only SERVES `rating.mmr` over its own edge and pulls
/// durable events), so the accepted `wiring` is unused today — the parameter exists
/// so every `cmd/*-svc` lib shares the one `modules(&ProcessWiring)` signature
