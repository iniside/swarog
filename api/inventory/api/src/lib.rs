...
//! and applies `#[rpc(prefix = "inventory")]` so the transport-FREE surface
//! (per-method wire envelopes, `METHOD_*` consts, `operations()`/`route_bindings()`)
//! is GENERATED into the child `holdings_rpc` module rather than hand-written. The
//! edge-dependent glue (`Client`, `register_server`, `provide_remote`) lives in the
//! sibling `inventoryrpc` crate, which expands this crate's metadata-callback macro
//! (`inventory_holdings_meta!`) — so THIS crate never depends on `edge`.
//!
