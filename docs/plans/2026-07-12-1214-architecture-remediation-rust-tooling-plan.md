...
6. Everything under `tools/` exists to develop, inspect, generate for, exercise, or verify the backend. Keep it simple and purpose-built; it is not a production service or security product and does not need certification, encrypted local control/state, custom cryptography, same-account adversarial defenses, reparse-point hardening, or daemon-grade protocols. Its acceptance criteria are backend coverage, accurate outcomes, bounded accidental-failure handling, exact owned-process cleanup, and no secrets in argv/logs/state. Add harder machinery only when a concrete backend-test failure requires it.
...
For config, render one coherent `{revision, settings}` and include `_expected_revision` as a hidden field. On submit, begin a transaction, first `LOCK TABLE config.settings IN SHARE ROW EXCLUSIVE MODE`, then lock/read the current revision, compare it with the posted expected revision, and return `SubmitError::Conflict` before any update/event if it changed. The table lock keeps the admin path's lock order compatible with ordinary/raw writers (`settings` DML then the revision trigger), covers inserts as well as existing rows, and avoids adding another trigger; it blocks writers only for the short DB batch, not readers. Apply the batch and commit once only when equal. This intentionally treats any intervening config change as a conflict because the form represents the whole snapshot.
...
Give `IpLimiter` a private `DEFAULT_MAX_VISITORS = 65_536` hard capacity used by HTTP gateway and admin login; provide a crate-private small-cap constructor for tests. Existing IPs continue through their existing bucket. A new IP at capacity is rejected immediately without a request-path scan or insertion. The existing once-per-minute background reap is the only reclamation mechanism; move admin login off its current every-256th-request scan by retaining the limiter on the module and starting that same reaper in lifecycle `start` (never `init`). Do not add a second coordinated capacity reaper or LRU eviction. Increment one global `http_rate_limit_table_saturated_total` counter owned by `httpmw` and registered into the existing private metrics registry without reversing the `metrics -> httpmw` dependency; saturation does not flip readiness.
