...
Expose an OS-authenticated local control endpoint: an owner-DACL named pipe on Windows and a mode-0600 Unix-domain socket with peer-credential validation on Linux. The state file stores only its address, never a bearer control token. `status` and `down` validate the live supervisor identity and endpoint ownership, then talk to it; they never kill recorded PIDs. On any child startup/health failure or unexpected later exit, mark the state, stop the successfully started prefix in reverse order, reap it, and exit nonzero. Ctrl-C and `down` perform the same bounded graceful teardown. The Windows Job Object or Linux guardian remains the crash safety net.

**Focused verification:** fake-service integration tests for environment isolation, topology switching, partial failure rollback, unexpected child exit, graceful/hung child, state progression after each spawn, stale-state decoy protection, lock contention, secret redaction, endpoint ACL/mode, and wrong-user/peer rejection. Then manually smoke `up monolith`, `status`, `down`, `up split`, and an injected failed service.
...
The initial exact stage order is: build, clippy with `-D warnings`, test, audit, fortress/archcheck, routecheck, split-proof; step 6 inserts the remaining custom stages in the fixed positions below. `verifyctl` owns the rollout lease for its whole run and lends one non-transferable handle only to splitproof. Blocking SKIP is legal only when the user explicitly selected `--no-install` and the required pinned tool is absent. Typed `NotApplicablePlatform` is also an accepted SKIP for a stage that cannot run on the current platform (notably fuzz on Windows), including under `--strict`; strict promotes an advisory `Fail`, not platform non-applicability. Cargo-audit install failure, invocation failure, or network failure is FAIL, never successful SKIP. No other advisory SKIP becomes green merely because the stage was hard to run.
