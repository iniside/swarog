...
fn rpc_module_hand_list_matches_filesystem() {
    ...
    let labels: Vec<&'static str> = modules.iter().map(|(l, _)| *l).collect();
    self_check_rpc_list(&labels).expect("rpc_modules() hand-list must match api/*/api");
}

/// The golden must cover both kinds: at least one `event` line (seven topics today)
/// and at least one `rpc` line (the HTTP-bound operations), so an accidentally
/// emptied source can't silently produce a trivially-matching golden.
#[test]
