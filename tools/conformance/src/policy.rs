...
pub fn input_policies() -> Vec<(InputKey, InputPolicy)> {
    ...
    vec![
        ...
        (key("admin.adminSubmit", "params.<value>", Wire), Validated { cap: apikeys::conformance::MAX_POLICY_BYTES, basis: "every module exposing adminapi::AdminSubmit caps its declared form values in Rust, before any SQL runs, against a mirroring column CHECK: wallet via admin::CATALOG_CAPS + the currencies_*_len_check constraints (widest 64) and its validate_movement caps, apikeys via store::COLUMN_CAPS + the roles_/keys_*_len_check constraints — MAX_NAME_BYTES (128) on every role/key name and MAX_POLICY_BYTES (4096, the widest declared form value ANY implementor posts) on a role policy — and notifications via service::validate_new inside its single insert authority, MAX_TITLE_BYTES (200) and MAX_BODY_BYTES (4000) against notifications_title_len_check/notifications_body_len_check. The 23514 mapping is NOT uniform and the difference is deliberate: wallet and apikeys map the CHECK back to the SAME operator-facing verdict because their writes can reach it, while notifications runs both caps in the same function that issues the INSERT, so nothing passing them reaches the CHECK and it stands only as the class fail-safe under them. Its one uncapped form value, player_id, is operator text bounded by the column's $1::uuid cast instead — the 22P02 comes back as Status::Invalid, never a row nobody owns. Two of the three claims are executable HERE: checks::ADMIN_SUBMIT_MODULES is diffed against modules/*/src before any assertion runs, so the implementor list cannot go stale, and every listed module must carry an input-byte-caps CapCase whose probe calls its real validator (today wallet::conformance -> validate_movement and apikeys::conformance -> store::validate_name/validate_policy), so the caps themselves are exercised. The third — that the cap runs on the SUBMIT path, pre-SQL, rather than only as the column CHECK — is NOT decided by this gate: it is pinned by each module's own tests (wallet's direct admin::check_catalog_caps/check_decimals_range tests, apikeys' store_tests over_cap_* writers), because in those two, through apply_submit, the Rust verdict and the CHECK's mapping are byte-identical and no input can separate them" }),
