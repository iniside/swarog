...
pub fn input_policies() -> Vec<(InputKey, InputPolicy)> {
    ...
    vec![
        ...
        (key("admin.adminSubmit", "params.<value>", Wire), Validated { cap: apikeys::conformance::MAX_POLICY_BYTES, basis: "every module exposing adminapi::AdminSubmit byte-checks its declared form values before SQL and maps the mirroring column CHECK's 23514 back to the same verdict: wallet via admin::CATALOG_CAPS + the currencies_*_len_check constraints (widest 64) and its validate_movement caps, apikeys via store::COLUMN_CAPS + the roles_/keys_*_len_check constraints — MAX_NAME_BYTES (128) on every role/key name and MAX_POLICY_BYTES (4096, the widest declared form value) on a role policy. This is a claim about EVERY implementor, so it is executable rather than prose: checks::ADMIN_SUBMIT_MODULES is diffed against modules/*/src before any assertion runs, and each listed module must carry an input-byte-caps CapCase" }),
